...
The distribution is bimodel.  Center is close to 1.
...
I'd assume it wouldn't be since it's not a normal distrubution.  Might lead to misleading information.
...
***Response**:
I think it is reasonable to make this assumption.  The sample size is large so we are able to assume normality.
...
```
**Response**: 90% confident the mean lies between 13.65 and 13.83
### Bootstrapping
...
```{r}
set.seed(950123)
sample_1000 <- map(1:1000, ~sample(gss_educ$educ, size = length(gss_educ$educ), replace = TRUE))
head(sample_1000)
```
```{r}
sample_1000_means <- map(sample_1000, ~mean(.))
head(sample_1000_means)
```
```{r}
sample_1000_means <- unlist(sample_1000_means)
head(sample_1000_means)
```
...
```{r}
quantile(sample_1000_means, probs = c(.05, .95))
```
We are 90% confident that the true population education years falls between 13.65 and 13.83 years.
Compare this bootstrap interval to your `t.test` confidence interval from above.
We see that the 90% confidence interval for both methods is almost identical.
...
```{r}
bootstrap_sampler <- function(data, n) {
  sample_n <- map(1:n, ~sample(data, size = length(data), replace = TRUE))
  mean <- map(sample_n, ~mean(.))
  median <- map(sample_n, ~median(.))
  midhinge <- map(sample_n, ~((quantile(., probs = .25) + quantile(., probs = .75)) / 2))
  output <- tibble(mean = unlist(mean), median = unlist(median), midhinge = unlist(midhinge))
}
```
```{r}
test <- bootstrap_sampler(gss_educ$educ, 1000)
head(test)
```
...
```{r}
bootstrap_data <- bootstrap_sampler(gss_educ$educ, 1000)
```
```{r}
quantile(bootstrap_data$mean, probs = c(.05, .95))
```
```{r}
quantile(bootstrap_data$median, probs = c(.05, .95))
```
```{r}
quantile(bootstrap_data$midhinge, probs = c(.05, .95))
```
...
**Response**:
I personally think that the median will be the best statistic.  There may or may not be outliers with values of 0 or high number of years of education, potentially causing the mean to trend in either direction. 
...
```{r}
new_gss <- gss %>% 
  mutate(advfront = ifelse(advfront %in% c("Strongly agree", "Agree"), "Yes", ifelse(advfront %in% c("Strongly disagree", "Disagree"), "No", advfront)))
head(new_gss)
```
...
```{r}
new_gss <- new_gss %>% 
  mutate(polviews = ifelse(polviews %in% c("Extremely liberal", "Liberal", "Slightly liberal"), "Liberal", ifelse(polviews %in% c("Extrmly conservative", "Conservative", "Slghtly conservative"), "Conservative", polviews)))
head(new_gss)
```
...
```{r}
filtered_new_gss <- new_gss %>% 
  filter(polviews %in% c("Liberal", "Conservative"), advfront %in% c("Yes", "No"))
head(filtered_new_gss)
```
...
```{r}
test2 <- filtered_new_gss %>% 
  mutate(combined = paste(polviews, ",", advfront))
```
```{r}
sample_n <- map(1:1000, ~sample(test2$combined, size = length(test2$combined), replace = TRUE))
```
```{r}
sample_liberal_yes <- map(sample_n, ~sum(str_count(., pattern = "Liberal , Yes")))
sample_liberal_no <- map(sample_n, ~sum(str_count(., pattern = "Liberal , No")))
sample_conservative_yes <- map(sample_n, ~sum(str_count(., pattern = "Conservative , Yes")))
sample_conservative_no <- map(sample_n, ~sum(str_count(., pattern = "Conservative , Yes")))
difference <- (unlist(sample_liberal_yes) / (unlist(sample_liberal_yes) + unlist(sample_liberal_no))) - (unlist(sample_conservative_yes) / (unlist(sample_conservative_yes) + unlist(sample_conservative_no)))
head(difference)
```
...
```{r}
quantile(difference, probs = c(.05, .95))
```
p(lib) - p(con)
We are 90% confident that the proportion differences between liberals and conservatives who support science research is between 35.68% and 41.17%.
## Attribution
